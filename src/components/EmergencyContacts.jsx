...
export default function EmergencyContacts() {
  ...
  return (
    <div className="bg-gradient-to-br from-red-50 to-orange-50 rounded-2xl shadow-lg border-2 border-red-200 overflow-hidden">
      <button
        onClick={() => setIsExpanded(!isExpanded)}
        className="w-full p-4 sm:p-5 lg:p-6 flex items-center justify-between hover:bg-red-100/50 active:bg-red-100/70 transition-colors min-h-[70px] sm:min-h-[80px] touch-manipulation"
        aria-expanded={isExpanded}
        aria-label="Toggle emergency contacts"
      >
        <div className="flex items-center space-x-3 sm:space-x-4 flex-1 min-w-0">
          <div className="bg-red-600 rounded-full p-2.5 sm:p-3 flex-shrink-0 shadow-md">
            <svg
        ...
        <div className="px-4 sm:px-5 lg:px-6 pb-4 sm:pb-5 lg:pb-6 pt-2">
          ...
          <div className="mt-4 sm:mt-5 lg:mt-6 p-3 sm:p-4 bg-white rounded-xl border border-gray-200 shadow-sm">
            <p className="text-xs sm:text-sm lg:text-base text-gray-600 text-center leading-relaxed">
              <span className="font-semibold text-red-600">⚠️ Important:</span> In case of emergency, 
              dial these numbers immediately. All services are available 24/7.
            </p>
          </div>
        </div>
      )}
